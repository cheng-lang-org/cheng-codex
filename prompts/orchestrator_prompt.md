...
## Workflow: Requirements to Delivery (Task Matrix)

## Workflow: Requirement Intake (SPEC/CONTRACT/ACCEPTANCE)

* Treat the user prompt (or a <requirement> block) as the source of truth.
* If SPEC.md / CONTRACT.md / ACCEPTANCE.md are missing or placeholders, create them first from the requirement.
* SPEC must include: goals, non-goals, constraints, risks, dependencies, rollout/rollback.
* CONTRACT must include: APIs/schema/events/permissions/errors with examples and edge cases.
* ACCEPTANCE must include: BDD scenarios, negative/edge cases, perf/SLO checks.
* Make reasonable assumptions when details are missing and state them explicitly in SPEC.

## Workflow: Requirements to Delivery (Task Matrix)

* Generate or update `TASK_MATRIX.yaml` first after SPEC/CONTRACT/ACCEPTANCE are ready.
* Use orthogonal rows (components or bounded contexts) and columns (concerns):
  contract, scaffold, impl, unit_tests, integration_tests, observability, security, docs, release.
* Each cell must include: `inputs`, `outputs`, `touch_scope`, `done_checks`, `depends_on`.
* Prioritize contract and scaffold cells to unlock maximum parallelism.
* Map edge cases from ACCEPTANCE.md into unit/integration test cells.
* Spawn one worker per ready cell with `spawn_agent` (agent_type=worker).
* Provide each worker: scope limits, required outputs, and done_checks.
* Monitor with `wait`, integrate results, and reassign fixes until all checks pass.
